...
class IndDataset(TensorDataset):

    def __init__(self, dataname, DATAPATH, config, verbose=False, get_info=False, longitudinal='follow'):
        self.IMGPATH = os.path.join(DATAPATH, dataname, "MRI")
        self.get_info = get_info
        self.config = config

        if dataname == 'OAS2' or dataname == 'SLIM':
            if longitudinal == 'base':
                self.df = pd.read_csv(os.path.join(DATAPATH, dataname, f"{dataname}_Phenotype_total_base.csv"))
            elif longitudinal == 'follow':
                self.df = pd.read_csv(os.path.join(DATAPATH, dataname, f"{dataname}_Phenotype_total_follow.csv"))

        else:
            self.df = pd.read_csv(os.path.join(DATAPATH, dataname, f"{dataname}_Phenotype_total.csv"))
